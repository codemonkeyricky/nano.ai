...
def main():
    ...
    for i in range(model.config.num_hidden_layers):
        ...
        if hasattr(model.model.layers[i], "linear_attn"):
            attn = model.model.layers[i].linear_attn
            with open(f"weights/layer_{i}_linear_attn_in_proj_qkvz_w.bin", "wb") as f:
                w = attn.in_proj_qkvz.weight.detach().cpu().view(torch.int16).numpy()
                w.tofile(f)
            with open(f"weights/layer_{i}_linear_attn_in_proj_ba_w.bin", "wb") as f:
                w = attn.in_proj_ba.weight.detach().cpu().view(torch.int16).numpy()
                w.tofile(f)
            ...
            with open(f"weights/layer_{i}_linear_attn_a_log.bin", "wb") as f:
                ...
                w.tofile(f)
            with open(f"weights/layer_{i}_linear_attn_norm.bin", "wb") as f:
                w = attn.norm.weight.detach().cpu().view(torch.int16).numpy()
                w.tofile(f)
            with open(f"weights/layer_{i}_linear_out_proj.bin", "wb") as f:
                w = attn.out_proj.weight.detach().cpu().view(torch.int16).numpy()
                w.tofile(f)
        ...
        if hasattr(model.model.layers[i], "self_attn"):
            ...
            with open(f"weights/layer_{i}_self_attn_q_proj_w.bin", "wb") as f:
                q = attn.q_proj.weight.detach().cpu().view(torch.int16).numpy()
                q.tofile(f)
            with open(f"weights/layer_{i}_self_attn_q_norm.bin", "wb") as f:
                q = attn.q_norm.weight.detach().cpu().view(torch.int16).numpy()
                q.tofile(f)
            with open(f"weights/layer_{i}_self_attn_k_proj_w.bin", "wb") as f:
                k = attn.k_proj.weight.detach().cpu().view(torch.int16).numpy()
                k.tofile(f)
            with open(f"weights/layer_{i}_self_attn_k_norm.bin", "wb") as f:
                k = attn.k_norm.weight.detach().cpu().view(torch.int16).numpy()
                k.tofile(f)
            with open(f"weights/layer_{i}_self_attn_v_proj_w.bin", "wb") as f:
                v = attn.v_proj.weight.detach().cpu().view(torch.int16).numpy()
                v.tofile(f)
            with open(f"weights/layer_{i}_self_attn_o_proj_w.bin", "wb") as f:
        ...
        if hasattr(model.model.layers[i].mlp, "experts"):
            shared_expert_gate = model.model.layers[i].mlp.shared_expert_gate
            gate = shared_expert_gate.weight.detach().cpu().view(torch.int16).numpy()
            with open(f"weights/layer_{i}_shared_expert_gate.bin", "wb") as f:
                gate.tofile(f)

            shared = model.model.layers[i].mlp.shared_expert
