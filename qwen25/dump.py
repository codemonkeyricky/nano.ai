...
def main():
    ...
    for i in range(model.config.num_hidden_layers):
        ...
        with open(f"qwen_weights/layer_{i}_input_layernorm.bin", "wb") as f:
        ...
        with open(f"qwen_weights/layer_{i}_post_attention_layernorm.bin", "wb") as f:
            norm_weight.tofile(f)

        # mlp weights
        gate_proj = model.model.layers[i].mlp.gate_proj.weight.detach().cpu().view(torch.int16).numpy()
        with open(f"qwen_weights/layer_{i}_mlp_gate_proj.bin", "wb") as f:
            gate_proj.tofile(f)

        up_proj = model.model.layers[i].mlp.up_proj.weight.detach().cpu().view(torch.int16).numpy()
        with open(f"qwen_weights/layer_{i}_mlp_up_proj.bin", "wb") as f:
            up_proj.tofile(f)

        down_proj = model.model.layers[i].mlp.down_proj.weight.detach().cpu().view(torch.int16).numpy()
        with open(f"qwen_weights/layer_{i}_mlp_down_proj.bin", "wb") as f:
            down_proj.tofile(f)

        # self_attn weights and biases
        attn = model.model.layers[i].self_attn

        with open(f"qwen_weights/layer_{i}_q_proj_w.bin", "wb") as f:
            q = attn.q_proj.weight.detach().cpu().view(torch.int16).numpy()
            q.tofile(f)
        with open(f"qwen_weights/layer_{i}_q_proj_b.bin", "wb") as f:
            q = attn.q_proj.bias.detach().cpu().view(torch.int16).numpy()
            q.tofile(f)
        with open(f"qwen_weights/layer_{i}_k_proj_w.bin", "wb") as f:
            k = attn.k_proj.weight.detach().cpu().view(torch.int16).numpy()
            k.tofile(f)
        with open(f"qwen_weights/layer_{i}_k_proj_b.bin", "wb") as f:
            k = attn.k_proj.bias.detach().cpu().view(torch.int16).numpy()
            k.tofile(f)
        with open(f"qwen_weights/layer_{i}_v_proj_w.bin", "wb") as f:
            v = attn.v_proj.weight.detach().cpu().view(torch.int16).numpy()
            v.tofile(f)
        with open(f"qwen_weights/layer_{i}_v_proj_b.bin", "wb") as f:
            v = attn.v_proj.bias.detach().cpu().view(torch.int16).numpy()
            v.tofile(f)
        with open(f"qwen_weights/layer_{i}_o_proj_w.bin", "wb") as f:
    ...
    if hasattr(model, "lm_head"):
        ...
        with open("qwen_weights/lm_head.bin", "wb") as f:
